        ...
        try {
            ...
            if ((!args || args[0] == undefined) && url == null) {
                const ErrorEmbed = new MessageEmbed()
                    .setDescription(`**Please enter a valid discussion message!\nUsage:**\`\`\`;discuss <message>\`\`\``)
                ...
                const SentEmbed = new MessageEmbed()
                    .setDescription(`**Your discussion message has been sent!  <:yes:798918006876799008>**`)
                    .setColor(`#${config["color"].default}`)
                message.channel.send({ embeds: [SentEmbed] })

                const DiscussEmbed = new MessageEmbed()
                    .setAuthor(`${message.author.tag}`, `${message.author.displayAvatarURL({dynamic: true})}`)
                    .setColor(`#${config["color"].discord}`)
                    text ? DiscussEmbed.setDescription(`**${discussion}**`) : null;
                    attachment ? DiscussEmbed.setImage(`${url}`) : null;
                await suggestionchannel.send({ embeds: [DiscussEmbed] });
                return;
            }
        } catch (error) {
            const ErrorEmbed = new MessageEmbed()
